...
export const generateLessonTeX = (content: string, subject: string, classLevel: string, week?: string, weekDate?: string): void => {
  const weekInfo = week ? `Week ${week}` : '';
  const dateInfo = weekDate ? ` (${weekDate})` : '';
...
\\pagestyle{fancy}
\\fancyhf{}
\\rhead{${subject} - ${classLevel}${weekInfo ? ` | ${weekInfo}${dateInfo}` : ''}}
\\lhead{Lesson Note}
\\rfoot{Page \\thepage}
...
\\large ${subject} - ${classLevel}${weekInfo ? ` \\\\ ${weekInfo}${dateInfo}` : ''}}
