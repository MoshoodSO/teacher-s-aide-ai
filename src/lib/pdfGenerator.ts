...
export const generateLessonPDF = ({ subject, classLevel, country, content, week, weekDate }: LessonPDFData): void => {
  const doc = new jsPDF({
  ...
  doc.text(`${country || 'Nigeria'} Curriculum${weekInfo ? ` | ${weekInfo}${dateInfo}` : ''}`, margin, 35);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text("Generated by Teacher's Aide", margin, pageHeight - 10);
...
export const generateLessonDOCX = (content: string, subject: string, classLevel: string, week?: string, weekDate?: string): void => {
  const weekInfo = week ? `Week ${week}` : '';
  const dateInfo = weekDate ? ` (${weekDate})` : '';
...
<head>
  <meta charset="utf-8">
  <title>${subject} - ${classLevel}${weekInfo ? ` ${weekInfo}` : ''} Lesson Note</title>
  <style>
...
<body>
  <div class="header">
    <h1>LESSON NOTE</h1>
    <p>${subject} - ${classLevel}${weekInfo ? ` | ${weekInfo}${dateInfo}` : ''}</p>
  </div>
  <pre style="white-space: pre-wrap; font-family: 'Times New Roman', serif;">${content}</pre>
  <div class="footer">Generated by Teacher's Aide</div>
</body>
</html>`;
...
export interface TeXExportResult {
  success: boolean;
  report: LatexRepairReport;
  message?: string;
}

const buildTeXDocument = (
  body: string,
  subject: string,
  classLevel: string,
  headerInfo: string,
...
\\pagestyle{fancy}
\\fancyhf{}
\\rhead{${subject} - ${classLevel}}
\\lhead{Lesson Note}
\\rfoot{Page \\thepage}
\\lfoot{Generated by Teacher's Aide}
...
\\large ${subject} - ${classLevel}${headerInfo}}
...
const formatTeXBody = (content: string): string =>
  content
    .split('\n')
    .map((line) => {
      ...
      if (trimmed.endsWith(':') && !trimmed.includes(': ')) {
        return `\\section*{${escapeTextOutsideMath(trimmed.replace(/:$/, ''))}}`;
      }
      if (trimmed.startsWith('Step ')) {
        return `\\subsection*{${escapeTextOutsideMath(trimmed)}}`;
      }
      if (trimmed.startsWith('-') || trimmed.startsWith('•')) {
        return `\\begin{itemize}\\item ${escapeTextOutsideMath(trimmed.replace(/^[-•]\s*/, ''))}\\end{itemize}`;
      }
      return escapeTextOutsideMath(trimmed);
    })
    .filter((line) => line !== '')
    .join('\n\n');

export const generateLessonTeX = (
  content: string,
  subject: string,
  classLevel: string,
  week?: string,
  ...
  const weekInfo = week ? `Week ${week}` : '';
  const dateInfo = weekDate ? ` (${weekDate})` : '';
  const headerInfo = weekInfo ? ` \\\\ ${weekInfo}${dateInfo}` : '';

  // Pipeline: generated content -> normalize/repair -> validate -> .tex -> download
  const report = normalizeLatexWithReport(content);
  ...
  const texContent = buildTeXDocument(
    formatTeXBody(report.normalized),
    subject,
    classLevel,
    headerInfo
  );

  const documentValidation = validateLatex(texContent);
  if (!documentValidation.valid) {
    return {
      success: false,
      report: { ...report, valid: false, errors: documentValidation.errors },
      message: documentValidation.errors[0],
    };
  }

  if (import.meta.env?.DEV) {
    console.debug('[LaTeX repair report]', {
